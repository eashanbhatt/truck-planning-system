"""
optimizer/src/reporter.py
--------------------------
Console output and CSV writers for the linear optimizer solution.

Outputs
-------
1. <output_dir>/milp_plan.csv     — full row-per-shipment load plan
2. <output_dir>/milp_savings.csv  — tiered savings summary
                                    (OVERALL / REGION / TRUCK / LTL)
"""
...
def print_results(plan: pd.DataFrame, baseline: float, elapsed: float) -> None:
    ...
    sep = "─" * 54
    print(f"\n{'═'*54}")
    print("  SOLUTION 2 — LINEAR OPTIMIZER (MILP)  (results)")
    print(f"{'═'*54}")
    print(f"\n  {'Metric':<36} {'Value':>15}")
    print(sep)
    ...
    print(f"  {'Solver':<36} {'PuLP + CBC (open-source)':>15}")
    print(f"  {'Optimisation method':<36} {'MILP per region':>15}")
    print(f"  {'Plan generated in':<36} {elapsed:>14.2f}s")
    print(f"{'═'*54}\n")

    # Per-region breakdown
    _print_region_table(plan)

    # Truck details
    if not tl_df.empty:
        truck_tbl = _truck_table(tl_df)
    ...
    if not ltl_df.empty:
        ...
        ltl_tbl.columns = ["Shipment", "Dest City", "ST", "Region",
...
def _print_region_table(plan: pd.DataFrame) -> None:
    grp = plan.groupby("region").agg(
        total     = ("shipment_id",   "count"),
        tl_count  = ("assigned_mode", lambda x: (x == "TL").sum()),
        ltl_count = ("assigned_mode", lambda x: (x == "LTL").sum()),
        opt_cost  = ("plan_cost",     "sum"),
        base_cost = ("ltl_cost_each", "sum"),
    ).reset_index()
    grp["savings"] = grp["base_cost"] - grp["opt_cost"]
    grp["pct"]     = (grp["savings"] / grp["base_cost"] * 100).round(1)

    out = grp.copy()
    for col in ["opt_cost", "base_cost", "savings"]:
        out[col] = out[col].map("${:,.0f}".format)
    out["pct"] = out["pct"].map("{:.1f}%".format)
    out.columns = ["Region", "Total", "→TL", "→LTL",
...
def _truck_table(tl_df: pd.DataFrame) -> pd.DataFrame:
    summary = (
    ...
    out = summary[["truck_id", "region", "n_shipments", "total_weight",
                   "n_stops", "tl_cost", "ltl_baseline", "savings",
                   "util_pct", "status"]].copy()
    ...
    out.columns = ["Truck", "Region", "Shpmnts", "Weight (lbs)",
                   "Stops", "TL Cost", "LTL Baseline", "Savings",
                   "Util%", "Status"]
    return out.sort_values("Savings", ascending=False).reset_index(drop=True)
...
PLAN_COLS = [
    ...
    "mode_flag", "region", "wh_to_dest_mi",
    "truck_id", "assigned_mode",
    "tl_cost", "ltl_cost_each", "plan_cost", "solver_status",
]
...
def save_savings(plan: pd.DataFrame, path: str) -> None:
    ...
    def _add(section, label, sub):
        ...
        opt      = sub["plan_cost"].sum()
        savings  = baseline - opt
